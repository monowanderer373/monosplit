...
export default defineConfig(({ mode }) => {
  const deploymentEnvironment = process.env.VITE_SENTRY_ENVIRONMENT
    ?? process.env.VERCEL_ENV
    ?? mode
  const deploymentRelease = process.env.VITE_SENTRY_RELEASE
    ?? (
      process.env.VERCEL_GIT_COMMIT_SHA
        ? `tabby-tally@${process.env.VERCEL_GIT_COMMIT_SHA}`
        : undefined
    )

  return {
    define: {
      __TABBY_TALLY_DEPLOYMENT_ENVIRONMENT__: JSON.stringify(deploymentEnvironment),
      __TABBY_TALLY_DEPLOYMENT_RELEASE__: JSON.stringify(deploymentRelease),
    },
    plugins: [
      react(),
      tailwindcss(),
